...
func RegisterUser(db *sql.DB) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		result, err := db.Exec("INSERT INTO users (email, password, name) VALUES (?, ?, ?)", user.Email, user.Password, user.Name)
		if err != nil {
			log.Println(err)
...
func CheckUser(db *sql.DB) gin.HandlerFunc {
	...
	return func(c *gin.Context) {
		...
		var count int
		err := db.QueryRow("SELECT COUNT(*) FROM users WHERE email = ? AND password = ?", user.Email, user.Password).Scan(&count)
		if err != nil {
			log.Println(err)
			c.JSON(http.StatusInternalServerError, gin.H{"error": "Internal server error"})
			return
